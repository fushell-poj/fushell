...
`NativeTooltip` supports `message`, `child`, `waitDuration`, `showDuration`, `exitDuration`, `preferBelow`, `verticalOffset`, `edgeGap`, `decoration`, `textStyle`, `padding`, `margin`, `constraints`, `excludeFromSemantics` and `animationDuration`. Nullable presentation/timing options resolve from the nearest `TooltipTheme` before desktop Material defaults. Default wait is 500 ms, exit delay 100 ms, focus display 1500 ms, and fade-in 150 ms. Set animationDuration to zero for immediate appearance; `MediaQuery.disableAnimations` also disables fading. Dismissal has no animation. Stationary hover is never truncated by showDuration; that duration limits keyboard-focus display. Focus on a descendant can trigger the tooltip, without introducing an extra tab stop.

`verticalOffset` follows Flutter Material Tooltip semantics: the target is the source rectangle **center**, not its bottom/top edge. Its explicit value, then `TooltipThemeData.verticalOffset`, then 24 logical pixels supplies a signed offset (+below/-above), with `PopupAnchor.none` and bottom/top gravity. For a true source-edge gap, pass `edgeGap: 8`: bottom/top anchoring with signed gap and the same `preferBelow` resolution. An explicit edgeGap overrides the theme verticalOffset; supplying both explicit options throws ArgumentError when the widget mounts or updates, including release builds. Negative or non-finite edgeGap also throws. Other theme properties are unchanged; no application theme extension is needed.

The source rectangle is clipped to its parent view and rounded inward to integer logical pixels. Center anchoring and integer offset rounding can differ from Flutter's floating-point placement by one logical pixel for an unclipped source; clipping can move the center further. Edge gap measures the popup surface, not painted bounds (tooltip margin can add visible spacing). Flip/slide constraints remain compositor-controlled; neither mode predicts other bars' reservations.
...
Anchors use the source RenderBox transform into its actual parent Flutter view, clipped and rounded inward to valid integer geometry. They are never global desktop coordinates or coordinates from another root. Invalid/offscreen anchors do not open. Placement allows compositor flipY, slideX and slideY, requests no grab, and always sets inputPassthrough. The verticalOffset or edgeGap displacement is signed in the initially preferred direction; xdg-positioner offsets are absolute and are **not inverted** by a compositor flip. Use `edgeGap: 0` for exact edge attachment where symmetric flipping matters (`verticalOffset: 0` attaches to the center). The widget dismisses on pointer down, parent metrics changes, source geometry changes on subsequent layout frames, presentation changes or unmount; it does not continuously schedule idle frames or silently reopen after movement. Re-enter to show again.
